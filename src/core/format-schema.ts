  ...
  z
    ...
    .refine(
      (s) => {
        ...
        return ['run', 'uses', 'agent', 'input', 'parallel', 'loop', 'foreach', 'widget'].filter(
          (k) => r[k] !== undefined,
        ).length === 1;
      },
      {
        message:
          'a step must have exactly one type key: run | uses | agent | input | parallel | loop | foreach | widget',
      },
    )
    .superRefine((s, ctx) => {
      const r = s as Record<string, unknown>;
